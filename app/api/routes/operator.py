...
async def summarize_service_tasks(
    ...
    failed_states = {TaskStatus.FAILED, TaskStatus.CANCELLED}
    pending_ids = [task.task_id for task in tasks if task.status in pending_states]
    success_ids = [task.task_id for task in tasks if task.status == TaskStatus.COMPLETED]
    failed_ids = [task.task_id for task in tasks if task.status in failed_states]
