...
def _build_client(api_base: str, operator_token: str) -> httpx.Client:
    base_url = api_base.rstrip("/")
    return httpx.Client(
        base_url=base_url, timeout=DEFAULT_TIMEOUT, headers={"X-Operator-Token": operator_token}
    )
...
def list_tasks(
    ...
    operator_token: str = typer.Option(
        ..., "--operator-token", envvar="DMS_OPERATOR_TOKEN", help="X-Operator-Token header value"
    ),
) -> None:
    """List tasks for a user."""
    with _build_client(api_base, operator_token) as client:
